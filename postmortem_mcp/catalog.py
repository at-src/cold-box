...
CATALOG: dict[str, ToolSpec] = {
    ...
    "android_probe": ToolSpec(
        name="android_probe",
        summary="Inventory Android mtd/sdcard images and parse acquisition notes",
        consumes=("case_directory", "android_mtd", "android_sdcard", "android_case_log"),
        produces=("android_inventory",),
        when_to_use="First step on DFRWS-style Android flash/sdcard acquisitions — map partitions and acquisition log",
        params=(_p("case_relpath", "string", True, "Android case directory relative to EVIDENCE_ROOT"),),
        feeds_rules=("R32",),
    ),
    "android_scan_artifacts": ToolSpec(
        name="android_scan_artifacts",
        summary="Scan Android mtd/sdcard for acquisition integrity and mobile artifacts",
        consumes=("case_directory", "android_mtd", "android_sdcard", "android_case_log"),
        produces=("android_findings",),
        when_to_use="Breadth sweep on Android cases — root exploit traces, ADB serial, SMS/telephony strings",
        params=(
            _p("search_root_relpath", "string", True, "Android evidence root relative to EVIDENCE_ROOT"),
            _p("max_records", "integer", False, "Cap findings returned"),
        ),
        mitre=("T1404", "T1430"),
        feeds_rules=("R32",),
    ),
    "macos_probe": ToolSpec(
        name="macos_probe",
        summary="Probe macOS AD1 custom-content image and companion manifest",
        consumes=("macos_ad1",),
        produces=("macos_inventory",),
        when_to_use="First step on macOS AD1/Spotlight cases — enumerate carved users and APFS volumes",
        params=(_p("artifact_relpath", "string", True, "AD1 image relative to EVIDENCE_ROOT"),),
        feeds_rules=("R33",),
    ),
    "macos_scan_artifacts": ToolSpec(
        name="macos_scan_artifacts",
        summary="Strings/manifest sweep of macOS AD1 for users, Spotlight, Safari, Slack",
        consumes=("macos_ad1",),
        produces=("macos_findings",),
        when_to_use="CyberDefenders Spotlight-style macOS triage on carved AD1 content",
        params=(
            _p("artifact_relpath", "string", True, "AD1 image relative to EVIDENCE_ROOT"),
            _p("max_records", "integer", False, "Cap findings returned"),
        ),
        mitre=("T1083", "T1564"),
        feeds_rules=("R33",),
