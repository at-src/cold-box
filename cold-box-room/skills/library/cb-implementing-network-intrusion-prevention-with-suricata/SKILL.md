---
name: cb-implementing-network-intrusion-prevention-with-suricata
skill_id: cb-implementing-network-intrusion-prevention-with-suricata
journal_id: CB-SKL-271
description: Cold-box analyst playbook — Implementing Network Intrusion Prevention
  With Suricata. Deploy and configure Suricata as a network intrusion prevention system
...
- inline-mode
cold_box_version: 2
inspired_by: implementing-network-intrusion-prevention-with-suricata
---

# Implementing Network Intrusion Prevention With Suricata (cold-box)

> **Journal ID:** `CB-SKL-271` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-271`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-network-intrusion-prevention-with-suricata")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-network-intrusion-prevention-with-suricata")` → note **`CB-SKL-271`**
2. `log_skill(case_id, journal_id="CB-SKL-271", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-271` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-271] powershell per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] head per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] sort per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] grep per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] tail per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] uniq per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] file per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-271] jq per playbook step",
  "why": "Executing cb-implementing-network-intrusion-prevention-with-suricata \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-271` (`cb-implementing-network-intrusion-prevention-with-suricata`)
