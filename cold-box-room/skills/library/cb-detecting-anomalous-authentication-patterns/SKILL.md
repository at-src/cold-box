---
name: cb-detecting-anomalous-authentication-patterns
skill_id: cb-detecting-anomalous-authentication-patterns
journal_id: CB-SKL-172
description: 'Cold-box analyst playbook — Detecting Anomalous Authentication Patterns.
  Detects anomalous authentication patterns using UEBA analytics, statistical baselines,
  and machine learning models to identify impossible travel, credential stuffing,
  brute force, password spraying, and compromised account behaviors across '
domain: cold-box
...
cold_box_version: 2
inspired_by: detecting-anomalous-authentication-patterns
---

# Detecting Anomalous Authentication Patterns (cold-box)

> **Journal ID:** `CB-SKL-172` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-172`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-anomalous-authentication-patterns")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-anomalous-authentication-patterns")` → note **`CB-SKL-172`**
2. `log_skill(case_id, journal_id="CB-SKL-172", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-172` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-172] head per playbook step",
  "why": "Executing cb-detecting-anomalous-authentication-patterns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-172] sort per playbook step",
  "why": "Executing cb-detecting-anomalous-authentication-patterns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-172] file per playbook step",
  "why": "Executing cb-detecting-anomalous-authentication-patterns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-172` (`cb-detecting-anomalous-authentication-patterns`)
