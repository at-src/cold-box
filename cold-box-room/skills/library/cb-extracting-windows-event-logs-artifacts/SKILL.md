---
name: cb-extracting-windows-event-logs-artifacts
skill_id: cb-extracting-windows-event-logs-artifacts
journal_id: CB-SKL-044
description: Cold-box analyst playbook — Extracting Windows Event Logs Artifacts.
  Extract, parse, and analyze Windows Event Logs (EVTX) using Chainsaw, Hayabusa,
  and EvtxECmd to detect lateral movement, persistence, and privilege escalation.
domain: cold-box
...
- incident-response
cold_box_version: 2
inspired_by: extracting-windows-event-logs-artifacts
---

# Extracting Windows Event Logs Artifacts (cold-box)

> **Journal ID:** `CB-SKL-044` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-044`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-extracting-windows-event-logs-artifacts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-extracting-windows-event-logs-artifacts")` → note **`CB-SKL-044`**
2. `log_skill(case_id, journal_id="CB-SKL-044", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-044` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-044] mmls per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] img_stat per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] powershell per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] sha256sum per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] hayabusa per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] EvtxECmd per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] mount per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-044] unzip per playbook step",
  "why": "Executing cb-extracting-windows-event-logs-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-044` (`cb-extracting-windows-event-logs-artifacts`)
