---
name: cb-deception-technology-deployment
skill_id: cb-deception-technology-deployment
journal_id: CB-SKL-165
description: Cold-box analyst playbook — Deception Technology Deployment. Deploys
  deception technology including honeypots, honeytokens, and decoy systems to detect
  attackers who have bypassed perimeter defenses, providing high-fidelity alerts with
  near-zero false positive rates. Use when SOC teams need early war
domain: cold-box
...
- detection
cold_box_version: 2
inspired_by: performing-deception-technology-deployment
---

# Deception Technology Deployment (cold-box)

> **Journal ID:** `CB-SKL-165` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-165`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-deception-technology-deployment")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-deception-technology-deployment")` → note **`CB-SKL-165`**
2. `log_skill(case_id, journal_id="CB-SKL-165", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-165` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-165] powershell per playbook step",
  "why": "Executing cb-deception-technology-deployment \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-165] find per playbook step",
  "why": "Executing cb-deception-technology-deployment \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-165] file per playbook step",
  "why": "Executing cb-deception-technology-deployment \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-165` (`cb-deception-technology-deployment`)
