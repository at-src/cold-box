---
name: cb-windows-lnk-files-for-artifacts
skill_id: cb-windows-lnk-files-for-artifacts
journal_id: CB-SKL-120
description: Cold-box analyst playbook — Windows Lnk Files For Artifacts. Parse Windows
  LNK shortcut files to extract target paths, timestamps, volume information, and
  machine identifiers for forensic timeline reconstruction.
domain: cold-box
...
- evidence-collection
cold_box_version: 2
inspired_by: analyzing-windows-lnk-files-for-artifacts
---

# Windows Lnk Files For Artifacts (cold-box)

> **Journal ID:** `CB-SKL-120` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-120`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-windows-lnk-files-for-artifacts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-windows-lnk-files-for-artifacts")` → note **`CB-SKL-120`**
2. `log_skill(case_id, journal_id="CB-SKL-120", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-120` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-120] mmls per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] img_stat per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] sha256sum per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] autopsy per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] JLECmd per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] mount per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] LECmd per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-120] find per playbook step",
  "why": "Executing cb-windows-lnk-files-for-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-120` (`cb-windows-lnk-files-for-artifacts`)
