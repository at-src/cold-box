---
name: cb-detecting-arp-poisoning-in-network-traffic
skill_id: cb-detecting-arp-poisoning-in-network-traffic
journal_id: CB-SKL-020
description: Cold-box analyst playbook — Detecting Arp Poisoning In Network Traffic.
...
- layer-2-security
cold_box_version: 2
inspired_by: detecting-arp-poisoning-in-network-traffic
---

# Detecting Arp Poisoning In Network Traffic (cold-box)

> **Journal ID:** `CB-SKL-020` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-020`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-arp-poisoning-in-network-traffic")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-arp-poisoning-in-network-traffic")` → note **`CB-SKL-020`**
2. `log_skill(case_id, journal_id="CB-SKL-020", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-020` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-020] wireshark per playbook step",
  "why": "Executing cb-detecting-arp-poisoning-in-network-traffic \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-020] tcpdump per playbook step",
  "why": "Executing cb-detecting-arp-poisoning-in-network-traffic \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-020] grep per playbook step",
  "why": "Executing cb-detecting-arp-poisoning-in-network-traffic \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-020] tail per playbook step",
  "why": "Executing cb-detecting-arp-poisoning-in-network-traffic \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-020] dd per playbook step",
  "why": "Executing cb-detecting-arp-poisoning-in-network-traffic \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-020` (`cb-detecting-arp-poisoning-in-network-traffic`)
