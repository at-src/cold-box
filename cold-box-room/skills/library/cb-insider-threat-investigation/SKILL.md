---
name: cb-insider-threat-investigation
skill_id: cb-insider-threat-investigation
journal_id: CB-SKL-066
description: Cold-box analyst playbook — Insider Threat Investigation. Investigates
  insider threat incidents involving employees, contractors, or trusted partners who
  misuse authorized access to steal data, sabotage systems, or violate security policies.
  Combines digital forensics, user behavior analytics, and
domain: cold-box
...
- DFIR
cold_box_version: 2
inspired_by: performing-insider-threat-investigation
---

# Insider Threat Investigation (cold-box)

> **Journal ID:** `CB-SKL-066` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-066`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-insider-threat-investigation")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-insider-threat-investigation")` → note **`CB-SKL-066`**
2. `log_skill(case_id, journal_id="CB-SKL-066", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-066` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-066] file per playbook step",
  "why": "Executing cb-insider-threat-investigation \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-066` (`cb-insider-threat-investigation`)
