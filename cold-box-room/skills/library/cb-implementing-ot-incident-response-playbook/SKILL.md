---
name: cb-implementing-ot-incident-response-playbook
skill_id: cb-implementing-ot-incident-response-playbook
journal_id: CB-SKL-059
description: Cold-box analyst playbook — Implementing Ot Incident Response Playbook.
  Develop and implement OT-specific incident response playbooks aligned with SANS
  PICERL framework, IEC 62443, and NIST SP 800-82 that address unique ICS challenges
  including safety-critical systems, limited downtime tolerance, and coordinati
domain: cold-box
...
- safety-critical
cold_box_version: 2
inspired_by: implementing-ot-incident-response-playbook
---

# Implementing Ot Incident Response Playbook (cold-box)

> **Journal ID:** `CB-SKL-059` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-059`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-ot-incident-response-playbook")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-ot-incident-response-playbook")` → note **`CB-SKL-059`**
2. `log_skill(case_id, journal_id="CB-SKL-059", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-059` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-059] file per playbook step",
  "why": "Executing cb-implementing-ot-incident-response-playbook \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-059` (`cb-implementing-ot-incident-response-playbook`)
