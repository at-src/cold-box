---
name: cb-ioc-enrichment-automation
skill_id: cb-ioc-enrichment-automation
journal_id: CB-SKL-067
description: Cold-box analyst playbook — Ioc Enrichment Automation. Automates Indicator
  of Compromise (IOC) enrichment by orchestrating lookups across VirusTotal, AbuseIPDB,
  Shodan, MISP, and other intelligence sources to provide contextual scoring and disposition
  recommendations. Use when SOC analysts need
domain: cold-box
...
- threat-intelligence
cold_box_version: 2
inspired_by: performing-ioc-enrichment-automation
---

# Ioc Enrichment Automation (cold-box)

> **Journal ID:** `CB-SKL-067` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-067`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-ioc-enrichment-automation")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-ioc-enrichment-automation")` → note **`CB-SKL-067`**
2. `log_skill(case_id, journal_id="CB-SKL-067", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-067` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-067] sort per playbook step",
  "why": "Executing cb-ioc-enrichment-automation \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-067] file per playbook step",
  "why": "Executing cb-ioc-enrichment-automation \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-067` (`cb-ioc-enrichment-automation`)
