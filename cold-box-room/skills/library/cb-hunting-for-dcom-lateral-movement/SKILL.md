---
name: cb-hunting-for-dcom-lateral-movement
skill_id: cb-hunting-for-dcom-lateral-movement
journal_id: CB-SKL-049
description: Cold-box analyst playbook — Hunting For Dcom Lateral Movement. Hunt for
  DCOM-based lateral movement by detecting abuse of MMC20.Application, ShellBrowserWindow,
  and ShellWindows COM objects through Sysmon Event ID 1 (process creation) and Event
  ID 3 (network connection) correlation, WMI event analysis,
domain: cold-box
...
- RPC
cold_box_version: 2
inspired_by: hunting-for-dcom-lateral-movement
---

# Hunting For Dcom Lateral Movement (cold-box)

> **Journal ID:** `CB-SKL-049` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-049`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-hunting-for-dcom-lateral-movement")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-hunting-for-dcom-lateral-movement")` → note **`CB-SKL-049`**
2. `log_skill(case_id, journal_id="CB-SKL-049", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-049` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-049] powershell per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] regsvr32 per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] head per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] sort per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] grep per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] zeek per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] find per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-049] uniq per playbook step",
  "why": "Executing cb-hunting-for-dcom-lateral-movement \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-049` (`cb-hunting-for-dcom-lateral-movement`)
