---
name: cb-implementing-next-generation-firewall-with-palo-alto
skill_id: cb-implementing-next-generation-firewall-with-palo-alto
journal_id: CB-SKL-273
description: Cold-box analyst playbook — Implementing Next Generation Firewall With
  Palo Alto. Configure and deploy Palo Alto Networks next-generation firewalls with
...
- zone-protection
cold_box_version: 2
inspired_by: implementing-next-generation-firewall-with-palo-alto
---

# Implementing Next Generation Firewall With Palo Alto (cold-box)

> **Journal ID:** `CB-SKL-273` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-273`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-next-generation-firewall-with-palo-alto")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-next-generation-firewall-with-palo-alto")` → note **`CB-SKL-273`**
2. `log_skill(case_id, journal_id="CB-SKL-273", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-273` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-273] find per playbook step",
  "why": "Executing cb-implementing-next-generation-firewall-with-palo-alto \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-273] file per playbook step",
  "why": "Executing cb-implementing-next-generation-firewall-with-palo-alto \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-273` (`cb-implementing-next-generation-firewall-with-palo-alto`)
