---
name: cb-implementing-taxii-server-with-opentaxii
skill_id: cb-implementing-taxii-server-with-opentaxii
journal_id: CB-SKL-286
description: Cold-box analyst playbook — Implementing Taxii Server With Opentaxii.
  Deploy and configure an OpenTAXII server to share and consume STIX-formatted cyber
  threat intelligence using the TAXII 2.1 protocol for automated indicator exchange
...
- automation
cold_box_version: 2
inspired_by: implementing-taxii-server-with-opentaxii
---

# Implementing Taxii Server With Opentaxii (cold-box)

> **Journal ID:** `CB-SKL-286` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-286`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-taxii-server-with-opentaxii")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-taxii-server-with-opentaxii")` → note **`CB-SKL-286`**
2. `log_skill(case_id, journal_id="CB-SKL-286", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-286` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-286] find per playbook step",
  "why": "Executing cb-implementing-taxii-server-with-opentaxii \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-286] file per playbook step",
  "why": "Executing cb-implementing-taxii-server-with-opentaxii \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-286` (`cb-implementing-taxii-server-with-opentaxii`)
