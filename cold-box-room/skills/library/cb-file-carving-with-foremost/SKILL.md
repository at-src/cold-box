---
name: cb-file-carving-with-foremost
skill_id: cb-file-carving-with-foremost
journal_id: CB-SKL-045
description: Cold-box analyst playbook — File Carving With Foremost. Recover files
  from disk images and unallocated space using Foremost's header-footer signature
  carving to extract evidence regardless of file system state.
domain: cold-box
...
- unallocated-space
cold_box_version: 2
inspired_by: performing-file-carving-with-foremost
---

# File Carving With Foremost (cold-box)

> **Journal ID:** `CB-SKL-045` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-045`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-file-carving-with-foremost")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-file-carving-with-foremost")` → note **`CB-SKL-045`**
2. `log_skill(case_id, journal_id="CB-SKL-045", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-045` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-045] img_stat per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] bulk_extractor per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] sha256sum per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] hashdeep per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] photorec per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] foremost per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] exiftool per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-045] scalpel per playbook step",
  "why": "Executing cb-file-carving-with-foremost \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-045` (`cb-file-carving-with-foremost`)
