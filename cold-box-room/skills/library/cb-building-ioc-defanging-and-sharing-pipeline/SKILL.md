---
name: cb-building-ioc-defanging-and-sharing-pipeline
skill_id: cb-building-ioc-defanging-and-sharing-pipeline
journal_id: CB-SKL-142
description: Cold-box analyst playbook — Building Ioc Defanging And Sharing Pipeline.
  Build an automated pipeline to defang indicators of compromise (URLs, IPs, domains,
  emails) for safe sharing and distribute them in STIX format through TAXII feeds
...
- threat-intelligence
cold_box_version: 2
inspired_by: building-ioc-defanging-and-sharing-pipeline
---

# Building Ioc Defanging And Sharing Pipeline (cold-box)

> **Journal ID:** `CB-SKL-142` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-142`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-building-ioc-defanging-and-sharing-pipeline")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-building-ioc-defanging-and-sharing-pipeline")` → note **`CB-SKL-142`**
2. `log_skill(case_id, journal_id="CB-SKL-142", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-142` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-142] file per playbook step",
  "why": "Executing cb-building-ioc-defanging-and-sharing-pipeline \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-142` (`cb-building-ioc-defanging-and-sharing-pipeline`)
