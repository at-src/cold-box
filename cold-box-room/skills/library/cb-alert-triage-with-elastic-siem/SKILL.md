---
name: cb-alert-triage-with-elastic-siem
skill_id: cb-alert-triage-with-elastic-siem
journal_id: CB-SKL-127
description: Cold-box analyst playbook — Alert Triage With Elastic Siem. Perform systematic
  alert triage in Elastic Security SIEM to rapidly classify, prioritize, and investigate
  security alerts for SOC operations.
domain: cold-box
...
- kibana
cold_box_version: 2
inspired_by: performing-alert-triage-with-elastic-siem
---

# Alert Triage With Elastic Siem (cold-box)

> **Journal ID:** `CB-SKL-127` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-127`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-alert-triage-with-elastic-siem")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-alert-triage-with-elastic-siem")` → note **`CB-SKL-127`**
2. `log_skill(case_id, journal_id="CB-SKL-127", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-127` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-127] sort per playbook step",
  "why": "Executing cb-alert-triage-with-elastic-siem \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-127] find per playbook step",
  "why": "Executing cb-alert-triage-with-elastic-siem \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-127] file per playbook step",
  "why": "Executing cb-alert-triage-with-elastic-siem \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-127` (`cb-alert-triage-with-elastic-siem`)
