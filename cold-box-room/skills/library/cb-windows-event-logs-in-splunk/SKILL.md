---
name: cb-windows-event-logs-in-splunk
skill_id: cb-windows-event-logs-in-splunk
journal_id: CB-SKL-119
description: Cold-box analyst playbook — Windows Event Logs In Splunk. Analyzes Windows
  Security, System, and Sysmon event logs in Splunk to detect authentication attacks,
  privilege escalation, persistence mechanisms, and lateral movement using SPL queries
  mapped to MITRE ATT&CK techniques. Use when SOC analys
domain: cold-box
...
- active-directory
cold_box_version: 2
inspired_by: analyzing-windows-event-logs-in-splunk
---

# Windows Event Logs In Splunk (cold-box)

> **Journal ID:** `CB-SKL-119` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-119`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-windows-event-logs-in-splunk")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-windows-event-logs-in-splunk")` → note **`CB-SKL-119`**
2. `log_skill(case_id, journal_id="CB-SKL-119", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-119` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-119] powershell per playbook step",
  "why": "Executing cb-windows-event-logs-in-splunk \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-119] sort per playbook step",
  "why": "Executing cb-windows-event-logs-in-splunk \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-119] file per playbook step",
  "why": "Executing cb-windows-event-logs-in-splunk \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-119` (`cb-windows-event-logs-in-splunk`)
