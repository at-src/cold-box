---
name: cb-detecting-compromised-cloud-credentials
skill_id: cb-detecting-compromised-cloud-credentials
journal_id: CB-SKL-178
description: 'Cold-box analyst playbook — Detecting Compromised Cloud Credentials.
  ...
  and credential abuse indicators using GuardDuty, Defender for Identity, '
domain: cold-box
...
- anomaly-detection
cold_box_version: 2
inspired_by: detecting-compromised-cloud-credentials
---

# Detecting Compromised Cloud Credentials (cold-box)

> **Journal ID:** `CB-SKL-178` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-178`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-compromised-cloud-credentials")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-compromised-cloud-credentials")` → note **`CB-SKL-178`**
2. `log_skill(case_id, journal_id="CB-SKL-178", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-178` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-178] sort per playbook step",
  "why": "Executing cb-detecting-compromised-cloud-credentials \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-178` (`cb-detecting-compromised-cloud-credentials`)
