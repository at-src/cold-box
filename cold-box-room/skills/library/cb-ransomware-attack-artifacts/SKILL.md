---
name: cb-ransomware-attack-artifacts
skill_id: cb-ransomware-attack-artifacts
journal_id: CB-SKL-104
description: Cold-box analyst playbook — Ransomware Attack Artifacts. Identify, collect,
  and analyze ransomware attack artifacts to determine the variant, initial access
  vector, encryption scope, and recovery options.
domain: cold-box
...
- evidence-collection
cold_box_version: 2
inspired_by: investigating-ransomware-attack-artifacts
---

# Ransomware Attack Artifacts (cold-box)

> **Journal ID:** `CB-SKL-104` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-104`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-ransomware-attack-artifacts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-ransomware-attack-artifacts")` → note **`CB-SKL-104`**
2. `log_skill(case_id, journal_id="CB-SKL-104", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-104` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-104] powershell per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] hayabusa per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] strings per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] PECmd per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] head per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] sort per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] grep per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-104] find per playbook step",
  "why": "Executing cb-ransomware-attack-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-104` (`cb-ransomware-attack-artifacts`)
