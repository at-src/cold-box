---
name: cb-typosquatting-domains-with-dnstwist
skill_id: cb-typosquatting-domains-with-dnstwist
journal_id: CB-SKL-337
description: Cold-box analyst playbook — Typosquatting Domains With Dnstwist. Detect
  typosquatting, homograph phishing, and brand impersonation domains using dnstwist
  to generate domain permutations and identify registered lookalike domains targeting
  your organization.
domain: cold-box
...
- threat-intelligence
cold_box_version: 2
inspired_by: analyzing-typosquatting-domains-with-dnstwist
---

# Typosquatting Domains With Dnstwist (cold-box)

> **Journal ID:** `CB-SKL-337` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-337`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-typosquatting-domains-with-dnstwist")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-typosquatting-domains-with-dnstwist")` → note **`CB-SKL-337`**
2. `log_skill(case_id, journal_id="CB-SKL-337", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-337` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-337] ssdeep per playbook step",
  "why": "Executing cb-typosquatting-domains-with-dnstwist \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-337] sort per playbook step",
  "why": "Executing cb-typosquatting-domains-with-dnstwist \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-337` (`cb-typosquatting-domains-with-dnstwist`)
