---
name: cb-detecting-network-anomalies-with-zeek
skill_id: cb-detecting-network-anomalies-with-zeek
journal_id: CB-SKL-198
description: Cold-box analyst playbook — Detecting Network Anomalies With Zeek. Deploys
  and configures Zeek (formerly Bro) network security monitor to passively analyze
  network traffic, generate structured logs, detect anomalous behavior, and create
  custom detection scripts for threat hunting and incident response.
domain: cold-box
...
- threat-hunting
cold_box_version: 2
inspired_by: detecting-network-anomalies-with-zeek
---

# Detecting Network Anomalies With Zeek (cold-box)

> **Journal ID:** `CB-SKL-198` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-198`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-network-anomalies-with-zeek")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-network-anomalies-with-zeek")` → note **`CB-SKL-198`**
2. `log_skill(case_id, journal_id="CB-SKL-198", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-198` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-198] head per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] sort per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] grep per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] zeek per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] find per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] uniq per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] file per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-198] awk per playbook step",
  "why": "Executing cb-detecting-network-anomalies-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-198` (`cb-detecting-network-anomalies-with-zeek`)
