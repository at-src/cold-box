---
name: cb-building-incident-timeline-with-timesketch
skill_id: cb-building-incident-timeline-with-timesketch
journal_id: CB-SKL-007
description: Cold-box analyst playbook — Building Incident Timeline With Timesketch.
  Build collaborative forensic incident timelines using Timesketch to ingest, normalize,
  and analyze multi-source event data for attack chain reconstruction and investigation
  documentation.
domain: cold-box
...
cold_box_version: 2
inspired_by: building-incident-timeline-with-timesketch
---

# Building Incident Timeline With Timesketch (cold-box)

> **Journal ID:** `CB-SKL-007` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-007`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-building-incident-timeline-with-timesketch")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-building-incident-timeline-with-timesketch")` → note **`CB-SKL-007`**
2. `log_skill(case_id, journal_id="CB-SKL-007", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-007` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-007] mmls per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] img_stat per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] log2timeline.py per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] powershell per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] find per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] stat per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-007] file per playbook step",
  "why": "Executing cb-building-incident-timeline-with-timesketch \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-007` (`cb-building-incident-timeline-with-timesketch`)
