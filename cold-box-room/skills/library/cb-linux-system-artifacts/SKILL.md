---
name: cb-linux-system-artifacts
skill_id: cb-linux-system-artifacts
journal_id: CB-SKL-073
description: Cold-box analyst playbook — Linux System Artifacts. Examine Linux system
  artifacts including auth logs, cron jobs, shell history, and system configuration
  to uncover evidence of compromise or unauthorized activity.
domain: cold-box
...
cold_box_version: 2
inspired_by: analyzing-linux-system-artifacts
---

# Linux System Artifacts (cold-box)

> **Journal ID:** `CB-SKL-073` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-073`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-linux-system-artifacts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-linux-system-artifacts")` → note **`CB-SKL-073`**
2. `log_skill(case_id, journal_id="CB-SKL-073", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-073` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-073] sha256sum per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] mount per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] head per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] grep per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] find per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] diff per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] file per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-073] zip per playbook step",
  "why": "Executing cb-linux-system-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-073` (`cb-linux-system-artifacts`)
