---
name: cb-configuring-suricata-for-network-monitoring
skill_id: cb-configuring-suricata-for-network-monitoring
journal_id: CB-SKL-160
description: Cold-box analyst playbook — Configuring Suricata For Network Monitoring.
...
- network-monitoring
cold_box_version: 2
inspired_by: configuring-suricata-for-network-monitoring
---

# Configuring Suricata For Network Monitoring (cold-box)

> **Journal ID:** `CB-SKL-160` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-160`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-configuring-suricata-for-network-monitoring")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-configuring-suricata-for-network-monitoring")` → note **`CB-SKL-160`**
2. `log_skill(case_id, journal_id="CB-SKL-160", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-160` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-160] wireshark per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] head per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] sort per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] grep per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] zeek per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] tail per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] uniq per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-160] file per playbook step",
  "why": "Executing cb-configuring-suricata-for-network-monitoring \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-160` (`cb-configuring-suricata-for-network-monitoring`)
