---
name: cb-implementing-honeypot-for-ransomware-detection
skill_id: cb-implementing-honeypot-for-ransomware-detection
journal_id: CB-SKL-265
description: Cold-box analyst playbook — Implementing Honeypot For Ransomware Detection.
  Deploys canary files, honeypot shares, and decoy systems to detect ransomware activity
  at the earliest possible stage. Configures canary tokens embedded in strategic file
  locations that trigger alerts when ransomware attempts encryption, us
domain: cold-box
...
- deception
cold_box_version: 2
inspired_by: implementing-honeypot-for-ransomware-detection
---

# Implementing Honeypot For Ransomware Detection (cold-box)

> **Journal ID:** `CB-SKL-265` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-265`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-honeypot-for-ransomware-detection")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-honeypot-for-ransomware-detection")` → note **`CB-SKL-265`**
2. `log_skill(case_id, journal_id="CB-SKL-265", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-265` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-265] powershell per playbook step",
  "why": "Executing cb-implementing-honeypot-for-ransomware-detection \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-265] file per playbook step",
  "why": "Executing cb-implementing-honeypot-for-ransomware-detection \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-265` (`cb-implementing-honeypot-for-ransomware-detection`)
