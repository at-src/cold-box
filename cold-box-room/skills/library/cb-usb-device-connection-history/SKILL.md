---
name: cb-usb-device-connection-history
skill_id: cb-usb-device-connection-history
journal_id: CB-SKL-116
description: Cold-box analyst playbook — Usb Device Connection History. Investigate
  USB device connection history from Windows registry, event logs, and setupapi logs
  to track removable media usage and potential data exfiltration.
domain: cold-box
...
- device-history
cold_box_version: 2
inspired_by: analyzing-usb-device-connection-history
---

# Usb Device Connection History (cold-box)

> **Journal ID:** `CB-SKL-116` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-116`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-usb-device-connection-history")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-usb-device-connection-history")` → note **`CB-SKL-116`**
2. `log_skill(case_id, journal_id="CB-SKL-116", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-116` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-116] mmls per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] img_stat per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] mount per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] sort per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] find per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] file per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-116] dd per playbook step",
  "why": "Executing cb-usb-device-connection-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-116` (`cb-usb-device-connection-history`)
