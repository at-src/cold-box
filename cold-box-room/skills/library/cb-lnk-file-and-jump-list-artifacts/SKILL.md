---
name: cb-lnk-file-and-jump-list-artifacts
skill_id: cb-lnk-file-and-jump-list-artifacts
journal_id: CB-SKL-074
description: Cold-box analyst playbook — Lnk File And Jump List Artifacts. Analyze
  Windows LNK shortcut files and Jump List artifacts to establish evidence of file
  access, program execution, and user activity using LECmd, JLECmd, and manual binary
  parsing of the Shell Link Binary format.
domain: cold-box
...
- recent-files
cold_box_version: 2
inspired_by: analyzing-lnk-file-and-jump-list-artifacts
---

# Lnk File And Jump List Artifacts (cold-box)

> **Journal ID:** `CB-SKL-074` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-074`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-lnk-file-and-jump-list-artifacts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-lnk-file-and-jump-list-artifacts")` → note **`CB-SKL-074`**
2. `log_skill(case_id, journal_id="CB-SKL-074", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-074` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-074] mmls per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-074] img_stat per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-074] powershell per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-074] JLECmd per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-074] LECmd per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-074] file per playbook step",
  "why": "Executing cb-lnk-file-and-jump-list-artifacts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-074` (`cb-lnk-file-and-jump-list-artifacts`)
