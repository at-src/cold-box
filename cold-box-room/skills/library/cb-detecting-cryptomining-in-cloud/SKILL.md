---
name: cb-detecting-cryptomining-in-cloud
skill_id: cb-detecting-cryptomining-in-cloud
journal_id: CB-SKL-180
description: Cold-box analyst playbook — Detecting Cryptomining In Cloud. This skill
  teaches security teams how to detect and respond to unauthorized cryptocurrency
  mining operations in cloud environments. It covers identifying cryptomining indicators
  through compute usage anomalies, network traffic patterns to m
domain: cold-box
...
- cost-anomaly
cold_box_version: 2
inspired_by: detecting-cryptomining-in-cloud
---

# Detecting Cryptomining In Cloud (cold-box)

> **Journal ID:** `CB-SKL-180` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-180`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-cryptomining-in-cloud")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-cryptomining-in-cloud")` → note **`CB-SKL-180`**
2. `log_skill(case_id, journal_id="CB-SKL-180", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-180` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-180] sort per playbook step",
  "why": "Executing cb-detecting-cryptomining-in-cloud \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-180] vol per playbook step",
  "why": "Executing cb-detecting-cryptomining-in-cloud \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-180] jq per playbook step",
  "why": "Executing cb-detecting-cryptomining-in-cloud \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-180` (`cb-detecting-cryptomining-in-cloud`)
