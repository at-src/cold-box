---
name: cb-detecting-port-scanning-with-fail2ban
skill_id: cb-detecting-port-scanning-with-fail2ban
journal_id: CB-SKL-202
description: Cold-box analyst playbook — Detecting Port Scanning With Fail2Ban. Configures
  Fail2ban with custom filters and actions to detect port scanning activity, SSH brute
  force attempts, and network reconnaissance, automatically banning offending IP addresses
  and alerting security teams to suspicious network probi
domain: cold-box
...
- automated-defense
cold_box_version: 2
inspired_by: detecting-port-scanning-with-fail2ban
---

# Detecting Port Scanning With Fail2Ban (cold-box)

> **Journal ID:** `CB-SKL-202` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-202`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-port-scanning-with-fail2ban")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-port-scanning-with-fail2ban")` → note **`CB-SKL-202`**
2. `log_skill(case_id, journal_id="CB-SKL-202", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-202` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-202] head per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] sort per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] grep per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] find per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] tail per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] uniq per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] file per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-202] awk per playbook step",
  "why": "Executing cb-detecting-port-scanning-with-fail2ban \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-202` (`cb-detecting-port-scanning-with-fail2ban`)
