---
name: cb-detecting-command-and-control-over-dns
skill_id: cb-detecting-command-and-control-over-dns
journal_id: CB-SKL-177
description: Cold-box analyst playbook — Detecting Command And Control Over Dns. Detects
  command-and-control (C2) communications tunneled through DNS protocol including
  DNS tunneling tools (Iodine, dnscat2, dns2tcp, Cobalt Strike DNS beacon), domain
  generation algorithms (DGA), encoded payload delivery via TXT/CNAME rec
domain: cold-box
...
- threat-detection
cold_box_version: 2
inspired_by: detecting-command-and-control-over-dns
---

# Detecting Command And Control Over Dns (cold-box)

> **Journal ID:** `CB-SKL-177` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-177`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-command-and-control-over-dns")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-command-and-control-over-dns")` → note **`CB-SKL-177`**
2. `log_skill(case_id, journal_id="CB-SKL-177", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-177` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-177] powershell per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] wireshark per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] strings per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] tshark per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] head per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] sort per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] grep per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-177] zeek per playbook step",
  "why": "Executing cb-detecting-command-and-control-over-dns \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-177` (`cb-detecting-command-and-control-over-dns`)
