---
name: cb-network-traffic-analysis-with-zeek
skill_id: cb-network-traffic-analysis-with-zeek
journal_id: CB-SKL-094
description: Cold-box analyst playbook — Network Traffic Analysis With Zeek. Deploy
  Zeek network security monitor to capture, parse, and analyze network traffic metadata
  for threat detection, anomaly identification, and forensic investigation.
domain: cold-box
...
- siem-integration
cold_box_version: 2
inspired_by: performing-network-traffic-analysis-with-zeek
---

# Network Traffic Analysis With Zeek (cold-box)

> **Journal ID:** `CB-SKL-094` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-094`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-network-traffic-analysis-with-zeek")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-network-traffic-analysis-with-zeek")` → note **`CB-SKL-094`**
2. `log_skill(case_id, journal_id="CB-SKL-094", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-094` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-094] head per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] sort per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] grep per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] zeek per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] find per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] diff per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] uniq per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-094] file per playbook step",
  "why": "Executing cb-network-traffic-analysis-with-zeek \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-094` (`cb-network-traffic-analysis-with-zeek`)
