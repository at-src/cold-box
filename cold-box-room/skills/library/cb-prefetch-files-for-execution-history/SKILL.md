---
name: cb-prefetch-files-for-execution-history
skill_id: cb-prefetch-files-for-execution-history
journal_id: CB-SKL-103
description: Cold-box analyst playbook — Prefetch Files For Execution History. Parse
  Windows Prefetch files to determine program execution history including run counts,
...
- evidence-collection
cold_box_version: 2
inspired_by: analyzing-prefetch-files-for-execution-history
---

# Prefetch Files For Execution History (cold-box)

> **Journal ID:** `CB-SKL-103` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-103`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-prefetch-files-for-execution-history")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-prefetch-files-for-execution-history")` → note **`CB-SKL-103`**
2. `log_skill(case_id, journal_id="CB-SKL-103", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-103` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-103] mmls per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] img_stat per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] powershell per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] sha256sum per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] procdump per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] regsvr32 per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] autopsy per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-103] mount per playbook step",
  "why": "Executing cb-prefetch-files-for-execution-history \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-103` (`cb-prefetch-files-for-execution-history`)
