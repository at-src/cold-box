---
name: cb-containing-active-breach
skill_id: cb-containing-active-breach
journal_id: CB-SKL-019
description: 'Cold-box analyst playbook — Containing Active Breach. Executes containment
  strategies to stop active adversary operations and prevent lateral movement during
  a confirmed security breach. Implements short-term and long-term containment using
  network segmentation, endpoint isolation, credential '
domain: cold-box
...
- live-response
cold_box_version: 2
inspired_by: containing-active-breach
---

# Containing Active Breach (cold-box)

> **Journal ID:** `CB-SKL-019` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-019`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-containing-active-breach")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-containing-active-breach")` → note **`CB-SKL-019`**
2. `log_skill(case_id, journal_id="CB-SKL-019", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-019` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-019] mmls per playbook step",
  "why": "Executing cb-containing-active-breach \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-019] img_stat per playbook step",
  "why": "Executing cb-containing-active-breach \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-019] powershell per playbook step",
  "why": "Executing cb-containing-active-breach \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-019] file per playbook step",
  "why": "Executing cb-containing-active-breach \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-019` (`cb-containing-active-breach`)
