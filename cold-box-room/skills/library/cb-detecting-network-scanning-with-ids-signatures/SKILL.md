---
name: cb-detecting-network-scanning-with-ids-signatures
skill_id: cb-detecting-network-scanning-with-ids-signatures
journal_id: CB-SKL-199
description: Cold-box analyst playbook — Detecting Network Scanning With Ids Signatures.
  Detect network reconnaissance and port scanning using Suricata and Snort IDS signatures,
  threshold-based detection rules, and traffic anomaly analysis to identify Nmap,
  Masscan, and custom scanning activity.
domain: cold-box
...
- threshold-rules
cold_box_version: 2
inspired_by: detecting-network-scanning-with-ids-signatures
---

# Detecting Network Scanning With Ids Signatures (cold-box)

> **Journal ID:** `CB-SKL-199` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-199`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-network-scanning-with-ids-signatures")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-network-scanning-with-ids-signatures")` → note **`CB-SKL-199`**
2. `log_skill(case_id, journal_id="CB-SKL-199", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-199` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-199] strings per playbook step",
  "why": "Executing cb-detecting-network-scanning-with-ids-signatures \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-199` (`cb-detecting-network-scanning-with-ids-signatures`)
