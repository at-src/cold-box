---
name: cb-sqlite-database-forensics
skill_id: cb-sqlite-database-forensics
journal_id: CB-SKL-108
description: Cold-box analyst playbook — Sqlite Database Forensics. Perform forensic
  analysis of SQLite databases to recover deleted records from freelists and WAL files,
  decode encoded timestamps, and extract evidence from browser history, messaging
  apps, and mobile device databases.
domain: cold-box
...
- unallocated-space
cold_box_version: 2
inspired_by: performing-sqlite-database-forensics
---

# Sqlite Database Forensics (cold-box)

> **Journal ID:** `CB-SKL-108` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-108`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-sqlite-database-forensics")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-sqlite-database-forensics")` → note **`CB-SKL-108`**
2. `log_skill(case_id, journal_id="CB-SKL-108", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-108` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-108] strings per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-108] sqlite3 per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-108] handle per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-108] file per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-108] zip per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-108] 7z per playbook step",
  "why": "Executing cb-sqlite-database-forensics \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-108` (`cb-sqlite-database-forensics`)
