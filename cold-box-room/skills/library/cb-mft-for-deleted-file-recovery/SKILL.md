---
name: cb-mft-for-deleted-file-recovery
skill_id: cb-mft-for-deleted-file-recovery
journal_id: CB-SKL-088
description: Cold-box analyst playbook — Mft For Deleted File Recovery. Analyze the
  NTFS Master File Table ($MFT) to recover metadata and content of deleted files by
  examining MFT record entries, $LogFile, $UsnJrnl, and MFT slack space using MFTECmd,
  analyzeMFT, and X-Ways Forensics.
domain: cold-box
...
- dfir
cold_box_version: 2
inspired_by: analyzing-mft-for-deleted-file-recovery
---

# Mft For Deleted File Recovery (cold-box)

> **Journal ID:** `CB-SKL-088` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-088`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-mft-for-deleted-file-recovery")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-mft-for-deleted-file-recovery")` → note **`CB-SKL-088`**
2. `log_skill(case_id, journal_id="CB-SKL-088", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-088` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-088] mmls per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] img_stat per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] powershell per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] analyzemft per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] MFTECmd per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] strings per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] mount per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-088] RBCmd per playbook step",
  "why": "Executing cb-mft-for-deleted-file-recovery \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-088` (`cb-mft-for-deleted-file-recovery`)
