---
name: cb-detecting-container-escape-attempts
skill_id: cb-detecting-container-escape-attempts
journal_id: CB-SKL-179
description: Cold-box analyst playbook — Detecting Container Escape Attempts. Container
  escape is a critical attack technique where an adversary breaks out of container
  isolation to access the host system or other containers. Detection involves monitoring
  for escape indicators
domain: cold-box
...
- escape-detection
cold_box_version: 2
inspired_by: detecting-container-escape-attempts
---

# Detecting Container Escape Attempts (cold-box)

> **Journal ID:** `CB-SKL-179` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-179`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-container-escape-attempts")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-container-escape-attempts")` → note **`CB-SKL-179`**
2. `log_skill(case_id, journal_id="CB-SKL-179", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-179` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-179] umount per playbook step",
  "why": "Executing cb-detecting-container-escape-attempts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-179] mount per playbook step",
  "why": "Executing cb-detecting-container-escape-attempts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-179] tail per playbook step",
  "why": "Executing cb-detecting-container-escape-attempts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-179] stat per playbook step",
  "why": "Executing cb-detecting-container-escape-attempts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-179] file per playbook step",
  "why": "Executing cb-detecting-container-escape-attempts \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-179` (`cb-detecting-container-escape-attempts`)
