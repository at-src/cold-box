---
name: cb-detecting-lateral-movement-in-network
skill_id: cb-detecting-lateral-movement-in-network
journal_id: CB-SKL-194
description: Cold-box analyst playbook — Detecting Lateral Movement In Network. Identifies
  lateral movement techniques in enterprise networks by analyzing authentication logs,
  network flows, SMB traffic, and RDP sessions using Zeek, Velociraptor, and SIEM
  correlation rules to detect attackers moving between systems.
domain: cold-box
...
- pass-the-hash
cold_box_version: 2
inspired_by: detecting-lateral-movement-in-network
---

# Detecting Lateral Movement In Network (cold-box)

> **Journal ID:** `CB-SKL-194` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-194`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-detecting-lateral-movement-in-network")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-detecting-lateral-movement-in-network")` → note **`CB-SKL-194`**
2. `log_skill(case_id, journal_id="CB-SKL-194", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-194` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-194] powershell per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] head per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] sort per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] grep per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] zeek per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] find per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] uniq per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-194] file per playbook step",
  "why": "Executing cb-detecting-lateral-movement-in-network \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-194` (`cb-detecting-lateral-movement-in-network`)
