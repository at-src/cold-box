---
name: cb-paste-site-monitoring-for-credentials
skill_id: cb-paste-site-monitoring-for-credentials
journal_id: CB-SKL-307
description: Cold-box analyst playbook — Paste Site Monitoring For Credentials. Monitor
  paste sites like Pastebin and GitHub Gists for leaked credentials, API keys, and
  sensitive data dumps using automated scraping and keyword matching to detect breaches
  early.
domain: cold-box
...
- early-warning
cold_box_version: 2
inspired_by: performing-paste-site-monitoring-for-credentials
---

# Paste Site Monitoring For Credentials (cold-box)

> **Journal ID:** `CB-SKL-307` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-307`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-paste-site-monitoring-for-credentials")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-paste-site-monitoring-for-credentials")` → note **`CB-SKL-307`**
2. `log_skill(case_id, journal_id="CB-SKL-307", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-307` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-307] strings per playbook step",
  "why": "Executing cb-paste-site-monitoring-for-credentials \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-307] file per playbook step",
  "why": "Executing cb-paste-site-monitoring-for-credentials \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-307` (`cb-paste-site-monitoring-for-credentials`)
