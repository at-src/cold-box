---
name: cb-implementing-api-key-security-controls
skill_id: cb-implementing-api-key-security-controls
journal_id: CB-SKL-252
description: Cold-box analyst playbook — Implementing Api Key Security Controls. Implements
  secure API key generation, storage, rotation, and revocation controls to protect
  API authentication credentials from leakage, brute force, and abuse. The engineer
  designs API key formats with sufficient entropy, implements secure
domain: cold-box
...
- secret-management
cold_box_version: 2
inspired_by: implementing-api-key-security-controls
---

# Implementing Api Key Security Controls (cold-box)

> **Journal ID:** `CB-SKL-252` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-252`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-implementing-api-key-security-controls")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-implementing-api-key-security-controls")` → note **`CB-SKL-252`**
2. `log_skill(case_id, journal_id="CB-SKL-252", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-252` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-252] strings per playbook step",
  "why": "Executing cb-implementing-api-key-security-controls \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-252] file per playbook step",
  "why": "Executing cb-implementing-api-key-security-controls \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-252` (`cb-implementing-api-key-security-controls`)
