---
name: cb-building-soc-playbook-for-ransomware
skill_id: cb-building-soc-playbook-for-ransomware
journal_id: CB-SKL-009
description: Cold-box analyst playbook — Building Soc Playbook For Ransomware. Builds
  a structured SOC incident response playbook for ransomware attacks covering detection,
  containment, eradication, and recovery phases with specific SIEM queries, isolation
  procedures, and decision trees. Use when SOC teams need formal
domain: cold-box
...
- containment
cold_box_version: 2
inspired_by: building-soc-playbook-for-ransomware
---

# Building Soc Playbook For Ransomware (cold-box)

> **Journal ID:** `CB-SKL-009` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-009`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-building-soc-playbook-for-ransomware")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-building-soc-playbook-for-ransomware")` → note **`CB-SKL-009`**
2. `log_skill(case_id, journal_id="CB-SKL-009", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-009` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-009] powershell per playbook step",
  "why": "Executing cb-building-soc-playbook-for-ransomware \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-009] wevtutil per playbook step",
  "why": "Executing cb-building-soc-playbook-for-ransomware \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-009] sort per playbook step",
  "why": "Executing cb-building-soc-playbook-for-ransomware \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-009] wmic per playbook step",
  "why": "Executing cb-building-soc-playbook-for-ransomware \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-009] file per playbook step",
  "why": "Executing cb-building-soc-playbook-for-ransomware \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-009` (`cb-building-soc-playbook-for-ransomware`)
