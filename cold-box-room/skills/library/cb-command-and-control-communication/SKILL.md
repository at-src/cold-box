---
name: cb-command-and-control-communication
skill_id: cb-command-and-control-communication
journal_id: CB-SKL-157
description: Cold-box analyst playbook — Command And Control Communication. Analyzes
  malware command-and-control (C2) communication protocols to understand beacon patterns,
  command structures, data encoding, and infrastructure. Covers HTTP, HTTPS, DNS,
  and custom protocol C2 analysis for detection development and t
domain: cold-box
...
- protocol-analysis
cold_box_version: 2
inspired_by: analyzing-command-and-control-communication
---

# Command And Control Communication (cold-box)

> **Journal ID:** `CB-SKL-157` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-157`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-command-and-control-communication")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-command-and-control-communication")` → note **`CB-SKL-157`**
2. `log_skill(case_id, journal_id="CB-SKL-157", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-157` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-157] powershell per playbook step",
  "why": "Executing cb-command-and-control-communication \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-157] wireshark per playbook step",
  "why": "Executing cb-command-and-control-communication \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-157] tshark per playbook step",
  "why": "Executing cb-command-and-control-communication \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-157] zeek per playbook step",
  "why": "Executing cb-command-and-control-communication \u2014 see Procedure section",
  "extra_args": []
}
```
...
```json
{
  ...
  "case_id": "{case_id}",
  "input_relpath": "{artifact_relpath}",
  "purpose": "[CB-SKL-157] file per playbook step",
  "why": "Executing cb-command-and-control-communication \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-157` (`cb-command-and-control-communication`)
