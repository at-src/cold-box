---
name: cb-ai-driven-osint-correlation
skill_id: cb-ai-driven-osint-correlation
journal_id: CB-SKL-126
description: Cold-box analyst playbook — Ai Driven Osint Correlation. Use AI and LLM-based
  reasoning to correlate findings across multiple OSINT sources—username enumeration,
  email lookups, social media profiles, domain records, breach databases, and dark-web
  mentions—into unified intelligence profiles with c
domain: cold-box
...
- maltego
cold_box_version: 2
inspired_by: performing-ai-driven-osint-correlation
---

# Ai Driven Osint Correlation (cold-box)

> **Journal ID:** `CB-SKL-126` — cite this in every journal entry while following this playbook.

## Cold-box constraints (always)

- Evidence lives on the **sealed operation table** — read only through the viewport (`input_relpath`).
- **Never write** on `operation-table/`; tool output goes to `records/{case_id}/scratch/`.
- **Log this playbook** in the case journal with **`CB-SKL-126`** (`log_skill`) when you adopt it and at each major step.
- Every SIFT execution: `run_sift_tool` with **`purpose`**, **`why`**, and cite returned **`tool_id`** + **`audit_id`** in the journal.
- Prefer `list_sift_tools(verified_only=true)` — skip `[BAD — DO NOT USE]` tools.
- Load this playbook with `get_skill("cb-ai-driven-osint-correlation")`; do not treat markdown code blocks as direct shell commands.

## Agent loop

1. `suggest_skills(case_profile=...)` or `get_skill("cb-ai-driven-osint-correlation")` → note **`CB-SKL-126`**
2. `log_skill(case_id, journal_id="CB-SKL-126", action="adopted", note="...")`
3. `list_sift_tools(verified_only=true)` → pick tools from the map below
4. `describe_sift_tool(tool_id)` → `run_sift_tool(...)` → journal stdout with `CB-SKL-126` + `audit_id`
...
```json
{
  ...
  "purpose": "[CB-SKL-126] jq per playbook step",
  "why": "Executing cb-ai-driven-osint-correlation \u2014 see Procedure section",
  "extra_args": []
}
```
...
```markdown
## {timestamp} — skill `CB-SKL-126` (`cb-ai-driven-osint-correlation`)
